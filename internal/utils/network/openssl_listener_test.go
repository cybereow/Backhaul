...
func negotiatedTLS13Cipher(t *testing.T, engine, certPath, keyPath string) uint16 {
	t.Helper()
	ln, err := NewTLSListener(engine, "127.0.0.1:0", []string{certPath}, []string{keyPath}, 0, 0, false)
	if err != nil {
...
func TestOpenSSLListenerServesTLS(t *testing.T) {
	certPath, keyPath := writeSelfSignedCert(t)
	ln, err := NewTLSListener(TLSEngineOpenSSL, "127.0.0.1:0", []string{certPath}, []string{keyPath}, 0, 0, false)
	if err != nil {
