...
// guarded by Conn.schedMu. rate is the EWMA throughput estimate; quar marks a
// leg quarantined (skipped for new sequential work); the in* fields track the
// chunk currently being written so the watchdog can re-send it elsewhere if the
// leg freezes.
...
// re-evaluates which legs are quarantined.
...
// reevaluateLocked quarantines every measured leg whose throughput has fallen
// below quarantineFraction of the fastest leg, and clears the flag on any leg
// that has recovered. An unmeasured leg (rate 0) is left active so it gets a
// chance to be measured. Caller holds schedMu.
func (c *Conn) reevaluateLocked() {
	...
	for i := range c.sched {
		r := c.sched[i].rate
		c.sched[i].quar = r > 0 && r < threshold
	}
...
func (c *Conn) scanStuck() {
	...
	for i := range c.sched {
		...
		if s.inFly && !s.inResent && now.Sub(s.inStart) > resendTimeout {
			...
			s.inResent = true
			s.quar = true // clearly frozen; keep new work off it
		}
	}
	c.ensureActiveLocked()
	c.schedMu.Unlock()
	...
	for _, job := range resends {
		...
		case <-c.closed:
			return
		default:
			// Queue full: drop the reroute. The original write is still in
			// flight and will deliver this sequence number; reroute is only an
			// optimization to deliver it sooner.
		}
	}
}
